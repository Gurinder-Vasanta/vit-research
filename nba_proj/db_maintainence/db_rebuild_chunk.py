...
projector.load_weights(config_chunks.PROJ_WEIGHTS)
...
def to_grayscale_3ch(frames_np):
    ...
    return gray_3ch.astype(np.float32)
...
# ------------------------------
vids = config_chunks.VIDS_TO_USE

def rebuild_db():
    ratt_db.delete(where={"vid_num": {"$ne": 'vid0'}})
    samples = dataset.load_samples(vids,stride=1,max_clips=config_chunks.NUM_CLIPS_PER_VID)
    chunk_samples = dataset.build_chunks(samples, chunk_size=config_chunks.CHUNK_SIZE)
