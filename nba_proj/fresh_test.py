...
embeddings_count = 0
while True:
    if(embeddings_count == max_frames): break #10240
    ret, frame = cap.read()
    if not ret: break
    global_counter+=1
    if(global_counter <= 3595): continue
    frames.append(frame)
    ...
    cv2.imwrite(f"data/unseen_test_images/ims/frame_{global_counter}.jpg", frame)
