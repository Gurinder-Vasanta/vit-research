...
# InputSpec(shape=(None, None, None, 3), ndim=4)
    ...
    if pooler == 'token':
      ...
      output_feature = x
      cls_token = tf.reduce_mean(x, axis=1)
    elif pooler == 'none':
      output_feature = x
      cls_token = tf.identity(x, name='encoded_tokens')
    else:
    ...
    if representation_size:
      pre_logits = layers.Dense(
          representation_size,
          kernel_regularizer=kernel_regularizer,
          name='pre_logits',
          kernel_initializer='lecun_normal' if original_init else 'he_uniform',
      )(cls_token)
      pre_logits = tf.nn.tanh(pre_logits)
    else:
      pre_logits = tf.identity(cls_token, name='pre_logits')

    endpoints['pre_logits'] = pre_logits
    
    if output_encoded_tokens:
      endpoints['encoded_tokens'] = output_feature  # (B, seq_len, D)
