...
class TransformerBlock(tf_keras.layers.Layer):
    def __init__(self, hidden_size, num_heads, inner_dim, dropout=0.1, temperature=1.0, **kwargs):
        ...
        self.ffn = tf_keras.Sequential([
            tf_keras.layers.Dense(inner_dim, activation='gelu'),
            tf_keras.layers.Dropout(dropout),
            tf_keras.layers.Dense(hidden_size),
            tf_keras.layers.Dropout(dropout),
        ])
        self.norm1 = tf_keras.layers.LayerNormalization(epsilon=1e-6)
        self.norm2 = tf_keras.layers.LayerNormalization(epsilon=1e-6)
        self.drop1 = tf_keras.layers.Dropout(dropout)
        self.drop2 = tf_keras.layers.Dropout(dropout)
    ...
    def call(self, inputs, training=False):
        ...
        # pre-norm + attention with temperature scaling
        x_norm = self.norm1(x)
        # x_scaled = x_norm / self.temperature
        x_scaled = x_norm / (self.temperature ** 0.5)
...
class RATTHeadV2(tf_keras.Model):
    # def _make_transformer_block(self, num_heads, hidden_size):
    #     return nn_blocks.TransformerEncoderBlock(
    #         inner_activation=activations.gelu,
    #         num_attention_heads=num_heads,
    #         inner_dim=hidden_size * 4,
    #         output_dropout=0.1,
    ...
    def __init__(self, hidden_size=768, num_heads=8, num_layers=2, mlp_dim=128):
        ...
        self.query_proj = tf_keras.Sequential([
        ...
        #     nn_blocks.TransformerEncoderBlock(
        #         inner_activation=activations.gelu,
        #         num_attention_heads=num_heads,
        #         inner_dim=hidden_size * 4,
        #         output_dropout=0.1,
        ...
        self.type_cls = self.add_weight(
            ...
            shape=(1, 1, hidden_size),
            # initializer="zeros",
            initializer=tf_keras.initializers.RandomNormal(stddev=0.2),
            trainable=True,
        )
        self.type_support_summary = self.add_weight(
            name="type_support_summary",
...
#         # positional embeddings — sized for K=30 each
#         self.support_pos = self.add_weight(
#             name="support_pos",
#             shape=(1, config.K_SIM, self.hidden_size),
#             initializer=tf.keras.initializers.RandomNormal(stddev=0.2),
...
#             support_proj_out = support_tokens
#             contrast_proj_out = contrast_tokens

#             cls = tf.repeat(self.cls_token, repeats=B, axis=0)

#             # todo: remove local since theres overreliance
#             # sequence: [cls, support x Ks, contrast x Kc, temporal x Kt, local]
#             x = tf.concat([
#                 cls,
#                 support_tokens,
#                 contrast_tokens,
...
#             seq_mask = tf.concat([
#                 ones_1,        # cls
#                 support_mask,  # support tokens
#                 contrast_mask, # contrast tokens
#                 temporal_mask, # temporal tokens
#                 # ones_1,        # local
#             ], axis=1)  # (B, seq_len)


#             # prevent CLS from attending to itself
#             seq_len = tf.shape(x)[1]
#             no_self_attn_mask = tf.ones((seq_len, seq_len), dtype=tf.bool)
#             no_self_attn_mask = tf.tensor_scatter_nd_update(
#                 no_self_attn_mask,
#                 indices=[[0, 0]],
#                 updates=[False]
#             )  # (seq_len, seq_len)
...
#             no_self_4d = no_self_attn_mask[tf.newaxis, tf.newaxis, :, :]  # (1, 1, seq_len, seq_len)
...
#             for i in range(self.num_layers):
#                 block = getattr(self, f"transformer_block_{i}")
#                 mask_4d = seq_mask[:, tf.newaxis, tf.newaxis, :]
#                 # x, attn = block([x, mask_4d], training=training)
...
#             # fixed positions — no summary tokens
#             idx_cls = 0
#             idx_support_start = 1
#             idx_contrast_start = 1 + Ks
#             idx_temporal_start = 1 + Ks + Kc
#             # idx_local = 1 + Ks + Kc + Kt
...
#             # attention logging
#             last_attn = attn_scores_all[-1]
#             attn_mean = tf.reduce_mean(last_attn, axis=1)
#             cls_attn = attn_mean[:, idx_cls, :]

#             tf.print("seq_mask real token count:", tf.reduce_sum(tf.cast(seq_mask, tf.int32), axis=1))
#             tf.print(
#                 "cls_attn | "
#                 "self:", tf.reduce_mean(cls_attn[:, idx_cls]),
#                 "sup:", tf.reduce_mean(tf.reduce_mean(cls_attn[:, idx_support_start:idx_support_start+Ks], axis=1)),
#                 "con:", tf.reduce_mean(tf.reduce_mean(cls_attn[:, idx_contrast_start:idx_contrast_start+Kc], axis=1)),
#                 "tmp:", tf.reduce_mean(tf.reduce_mean(cls_attn[:, idx_temporal_start:idx_temporal_start+Kt], axis=1)),
#                 # "local:", tf.reduce_mean(cls_attn[:, idx_local]),
#                 summarize=-1,
#             )
...
#             return class_logit, cls_out, aux
    ...
    #     # capture projected tokens before positional embeddings for aux
    #     support_proj_out = support_tokens
    #     contrast_proj_out = contrast_tokens
    ...
    #         "cls_attn | "
    #         "self:", tf.reduce_mean(cls_attn[:, idx_cls]),
    #         "sup_sum:", tf.reduce_mean(cls_attn[:, idx_support_summary]),
    #         "sup:", tf.reduce_mean(tf.reduce_mean(cls_attn[:, 2:2+Ks], axis=1)),
    #         "con_sum:", tf.reduce_mean(cls_attn[:, idx_contrast_summary]),
    #         "con:", tf.reduce_mean(tf.reduce_mean(cls_attn[:, 3+Ks:3+Ks+Kc], axis=1)),
    #         "tmp_sum:", tf.reduce_mean(cls_attn[:, idx_temporal_summary]),
    #         "tmp:", tf.reduce_mean(tf.reduce_mean(cls_attn[:, 4+Ks+Kc:4+Ks+Kc+Kt], axis=1)),
    #         "local:", tf.reduce_mean(cls_attn[:, idx_local]),
    #         summarize=-1,
    #     )
