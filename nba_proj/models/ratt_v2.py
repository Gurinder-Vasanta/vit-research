...
class TransformerBlock(tf_keras.layers.Layer):
    """Standard transformer encoder block using tf_keras native layers."""
    def __init__(self, hidden_size, num_heads, mlp_dim, dropout_rate=0.1, **kwargs):
...
class RATTHeadV2(tf_keras.Model):
    def __init__(self, hidden_size=768, num_heads=8, num_layers=2, mlp_dim=128):
        ...
        self.query_proj   = tf_keras.Sequential([layers.Dense(hidden_size)], name="query_proj")
        self.support_proj = make_proj("support_proj")
        self.contrast_proj = make_proj("contrast_proj")
        self.temporal_proj = make_proj("temporal_proj")
        ...
        # learned special tokens
        self.cls_token      = self.add_weight(name="cls_token",      shape=(1, 1, hidden_size), initializer="random_normal", trainable=True)
        self.support_token  = self.add_weight(name="support_token",  shape=(1, 1, hidden_size), initializer="random_normal", trainable=True)
        self.contrast_token = self.add_weight(name="contrast_token", shape=(1, 1, hidden_size), initializer="random_normal", trainable=True)
        self.temporal_token = self.add_weight(name="temporal_token", shape=(1, 1, hidden_size), initializer="random_normal", trainable=True)

        # type embeddings
        self.type_cls               = self.add_weight(name="type_cls",               shape=(1, 1, hidden_size), initializer=tf.keras.initializers.RandomNormal(stddev=0.02), trainable=True)
        self.type_support_summary   = self.add_weight(name="type_support_summary",   shape=(1, 1, hidden_size), initializer=tf.keras.initializers.RandomNormal(stddev=0.02), trainable=True)
        self.type_support           = self.add_weight(name="type_support",           shape=(1, 1, hidden_size), initializer=tf.keras.initializers.RandomNormal(stddev=0.02), trainable=True)
        self.type_contrast_summary  = self.add_weight(name="type_contrast_summary",  shape=(1, 1, hidden_size), initializer=tf.keras.initializers.RandomNormal(stddev=0.02), trainable=True)
        self.type_contrast          = self.add_weight(name="type_contrast",          shape=(1, 1, hidden_size), initializer=tf.keras.initializers.RandomNormal(stddev=0.02), trainable=True)
        self.type_temporal_summary  = self.add_weight(name="type_temporal_summary",  shape=(1, 1, hidden_size), initializer=tf.keras.initializers.RandomNormal(stddev=0.02), trainable=True)
        self.type_temporal          = self.add_weight(name="type_temporal",          shape=(1, 1, hidden_size), initializer=tf.keras.initializers.RandomNormal(stddev=0.02), trainable=True)
        self.type_local             = self.add_weight(name="type_local",             shape=(1, 1, hidden_size), initializer=tf.keras.initializers.RandomNormal(stddev=0.02), trainable=True)
    ...
    def call(
        ...
        x = tf.concat([
            cls, sup_summary, support_tokens,
            con_summary, contrast_tokens,
            tmp_summary, temporal_tokens,
            local,
        ], axis=1)
        ...
        x = x + tf.concat(type_parts, axis=1)
        ...
        last_attn    = attn_scores_all[-1]              # (B, heads, T, T)
        attn_mean    = tf.reduce_mean(last_attn, axis=1) # (B, T, T)
        cls_attn     = attn_mean[:, idx_cls, :]          # (B, T)
