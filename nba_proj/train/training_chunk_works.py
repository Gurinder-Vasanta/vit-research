...
#     pos_sim = tf.reduce_max(
#         tf.reduce_sum(q[:, None, :] * retrieved, axis=-1),
#         axis=1
#     )
#     pull = 1.0 - pos_sim   # (B,)
...
#     neg_sim = tf.reduce_mean(
#         tf.reduce_sum(q * tf.roll(r_mean, 1, axis=0), axis=-1)
#     )
#     push = neg_sim         # (B,)
...
def train_step(rag_head, proj_head, retriever, optimizer, loss_fn,
               frames, metadata, labels, accum,contrastive_coefficient, disable_cls):
    ...
    with tf.GradientTape() as tape:
        ...
        # combine them
        # loss = loss_cls + contrastive_coefficient * loss_contrast + 0.1 * loss_ibn        # λ = 0.1 to start
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------
    train_vids = config_chunks.VIDS_TO_USE
    ...
    train_chunks = chunk_samples[config_chunks.START_CHUNK_TRAIN:config_chunks.END_CHUNK_TRAIN] #was 2000
    val_chunks = chunk_samples[config_chunks.START_CHUNK_VALID:config_chunks.END_CHUNK_VALID]
    ...
    # 2. Build TF datasets
    # ---------------------------------------------
    train_dataset = build_tf_dataset_chunks(train_chunks, batch_size=config_chunks.CHUNK_BATCH_SIZE)
    val_dataset   = build_tf_dataset_chunks(val_chunks,   batch_size=config_chunks.CHUNK_BATCH_SIZE)
    ...
    # RAG head (trainable)
    ratt_head = RATTHead(hidden_size=768, num_queries=config_chunks.NUM_QUERIES, num_layers=config_chunks.NUM_LAYERS,num_heads=config_chunks.NUM_HEADS)
    ...
    retriever = RattChunkRetriever(collection, top_k=config_chunks.TOP_K, search_k=config_chunks.SEARCH_K)
    ...
    # 4. Optimizer / loss
    # ---------------------------------------------
    optimizer = tf.keras.optimizers.Adam(config_chunks.PHASE_1_LEARNING_RATE)
    ...
    accum_steps = config_chunks.ACCUM_BATCH_SIZE  # effective batch = physical batch * accum_steps
    ...
    for epoch in range(1,EPOCHS+1):
        ...
        if(epoch >= int(EPOCHS/2)+1): 
            optimizer.learning_rate.assign(config_chunks.PHASE_2_LEARNING_RATE)
            # contrastive_coefficient = config.PHASE_2_CONTRASTIVE_LOSS
        else: 
            optimizer.learning_rate.assign(config_chunks.PHASE_1_LEARNING_RATE)
        ...
        for frames_batch, metadata_batch, labels_batch in train_dataset:
            curloss, curacc = train_step(
                ...
                accum,contrastive_coefficient,disable_cls
            )

            batch_counter += 1
            losses.append(curloss)
            accs.append(curacc)
            if(batch_counter % config_chunks.PRINT_EVERY == 0):
        ...
        proj_head.save_weights(config_chunks.PROJ_WEIGHTS)
        ratt_head.save_weights(config_chunks.RATT_WEIGHTS)
        # validation at end of every epoch
        evaluate(val_dataset, ratt_head, proj_head, retriever, bce)
        # rebuild_db()
        if(epoch % config_chunks.ADJUST_CONTRASTIVE_LOSS_EVERY == 0 and epoch >= config_chunks.ADJUST_CONTRASTIVE_LOSS_EVERY): 
            contrastive_coefficient += config_chunks.INCREMENT_CONTRASTIVE_LOSS_BY
        if((epoch) % config_chunks.REBUILD_EVERY == 0 and (epoch) >= config_chunks.REBUILD_EVERY): 
            rebuild_db()
